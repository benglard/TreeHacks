...
function vars_opr(hardness)
  ...
  if random(2) == 1 then
    eval = a.eval + b.eval
    code = {string.format("int %s=%s;", var, a.expr)}
...
function small_loop_opr(hardness)
  ...
  --[[local code = {string.format("%s=%s", var, a.expr),
                string.format("for x in range(%d):%s%s=%s", loop, var,
                                                            op, b.expr, var)}]]--
  local code = {
    string.format("int %s=%s;", var, a.expr),
    string.format("for(int i=0;i<%d;i++){%s%s=%s;}", loop, var, op, b.expr, var)
  }
  local expr = var
  return code, expr, eval
end
...
function ifstat_opr(hardness)
  ...
  local expr = string.format("%s%s%s?%s:%s", a.expr, name, b.expr, c.expr, d.expr)
  return {}, expr, output
end
